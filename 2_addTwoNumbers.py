...
class Solution:
    def addTwoNumbers(self, l1: Optional[ListNode], l2: Optional[ListNode]) -> Optional[ListNode]:
        ...
        while True:
            ...
            if sum > 9:
                carry = 1
                l3_dummy.next = ListNode(sum-10)
            else:
                carry = 0
                l3_dummy.next = ListNode(sum)
                
            l3_dummy = l3_dummy.next
            # 3. Move along to next node in l1 and l2, if at the end then assigne them the value of 0
